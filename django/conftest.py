...
# Generate a fresh RSA key pair for the test session so JWT token generation
# and verification work correctly without needing JWT_PRIVATE_KEY / JWT_PUBLIC_KEY
# environment variables set in CI.  Using the real RS256 flow means the full
# sign→verify round-trip is tested exactly as it runs in production.
...
settings.JWT_PRIVATE_KEY = _rsa_key.private_bytes(
    encoding=serialization.Encoding.PEM,
    format=serialization.PrivateFormat.TraditionalOpenSSL,
    encryption_algorithm=serialization.NoEncryption(),
).decode()
settings.JWT_PUBLIC_KEY = _rsa_key.public_key().public_bytes(
    encoding=serialization.Encoding.PEM,
