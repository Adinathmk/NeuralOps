...
class RazorpayWebhookView(APIView):
    ...
    def post(self, request):
        ...
        event_id = (
            ...
            or payload.get("account_id", "") + event_name
        )
        ...
        BillingEvent.objects.create(
            razorpay_event_id=event_id, event_type=event_name, payload=payload
        )
        ...
        if event_name in ["subscription.charged", "subscription.activated"]:
            plan_tier = subscription_payload.get("notes", {}).get(
            ...
            if free_quota.max_users != -1:
                active_users = list(
                    User.objects.filter(tenant=tenant, is_active=True).order_by(
                        "created_at"
                    )
                )
            ...
            if free_quota.max_api_keys != -1:
                active_keys = list(
                    APIKey.objects.filter(tenant=tenant, is_active=True).order_by(
                        "created_at"
                    )
                )
