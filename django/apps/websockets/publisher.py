...
def _publish_to_sqs(
    ...
    if not queue_url:
        logger.warning(
            "SQS_PUSH_INCIDENTS_QUEUE_URL not configured. Skipping SQS push."
        )
        return
    ...
    try:
        ...
        logger.info(
            f"Queued push notification: incident={incident_id} tenant={tenant_id}"
        )
...
def push_incident_update(incident_id: str, data: dict):
    ...
    async_to_sync(channel_layer.group_send)(
        f"incident_{incident_id}", {"type": "incident.update", "data": data}
    )
...
def push_incident_analysis_complete(incident_id: str, tenant_id: str, data: dict):
    ...
    channel_layer = get_channel_layer()
    async_to_sync(channel_layer.group_send)(
        f"incident_{incident_id}", {"type": "incident.analysis_complete", "data": data}
    )
