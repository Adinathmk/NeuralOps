...
from app.services.embedding_service import build_query_embed_text, embed_text
...
async def main():
    ...
    results = await search_similar_playbooks(
        tenant_id=tenant_id, query_vector=query_vector, top_k=3, distance_threshold=0.60
    )
        ...
        async with AsyncSessionLocal() as session:
            for rank, result in enumerate(results, 1):
                ...
                db_res = await session.execute(
                    text(
                        "SELECT error_pattern, instructions FROM playbook_snapshots WHERE playbook_id = :pid"
                    ),
