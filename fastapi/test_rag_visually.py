...
async def main():
    ...
    async with AsyncSessionLocal() as session:
        ...
        await session.commit()
        print(
            "✅ Successfully generated 768-dimensional vectors for all playbooks and stored them in pgvector!"
        )
        ...
        if not results:
            ...
            for rank, result in enumerate(results, 1):
                ...
                matched_pb = next(
                    p for p in playbooks if str(p["id"]) == result["playbook_id"]
                )
