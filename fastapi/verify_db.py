...
async def run():
    async with AsyncSessionLocal() as session:
        ...
        for t in tenants:
            print(
                f"ID: {t.tenant_id} | Repo: {t.github_repo_owner}/{t.github_repo_name} | Status: {t.github_indexing_status} | Commit: {t.github_last_indexed_commit}"
            )
        ...
        for s in symbols:
            print(
                f"- Symbol: {s.symbol_name:18} | Type: {s.chunk_type:8} | File: {s.file_path} | Commit: {s.last_commit}"
            )
